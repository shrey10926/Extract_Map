...
Features
    ...
    * Side-by-side display of PC time vs. the time sent to the master, with the raw
      payload shown beneath the sent time.
    * JSON config in %APPDATA%\\RenesasTimeSync for protocol + timing/resilience knobs.
    * Rotating file logging in %APPDATA%\\RenesasTimeSync\\logs and defensive error
      handling (per-connection, per-thread, and a global excepthook).
...
class StreamThread(QThread):
    ...
    Emits `status(state, message)` for the indicator and `frame(time, payload)` for
    each frame actually sent. Reuses the backend's build_payload() and socket tuning.
    """

    status = pyqtSignal(str, str)  # state in: connecting|connected|disconnected|error|stopped
    frame = pyqtSignal(str, str)   # (sent HH:MM:SS, payload display text)
    ...
    def _send_loop(self, sock: socket.socket) -> None:
        ...
        while not self._stop.is_set():
            ...
            count += 1
            payload_text = payload.decode("ascii", "replace").replace("\r", "\\r")
            self.frame.emit(now.strftime("%H:%M:%S"), payload_text)
...
QLabel#caption { color: #7B8794; font-size: 9pt; font-weight: 700; }
QLabel#bigtime { color: #1F2933; font-family: Consolas, 'Courier New', monospace; font-size: 46px; font-weight: 600; }
QLabel#subtime { color: #7B8794; font-family: Consolas, 'Courier New', monospace; font-size: 11pt; }
QLabel#payload { color: #3E4C59; font-family: Consolas, 'Courier New', monospace; font-size: 10pt; }
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        root.addLayout(self._build_cards(), 1)
    ...
    def _make_card(self, caption: str, sub_object: str):
        card = QFrame()
    ...
    def _build_cards(self) -> QHBoxLayout:
        ...
        pc_card, self.pc_time, self.pc_sub = self._make_card("PC TIME (SYSTEM)", "subtime")
        sent_card, self.sent_time, self.sent_sub = self._make_card(
            "SENT TO MASTER", "payload"
        )

        cards.addWidget(pc_card, 1)
        cards.addWidget(sent_card, 1)
        return cards
    ...
    def on_frame(self, time_str: str, payload_str: str) -> None:
        self.sent_time.setText(time_str)
        self.sent_sub.setText(f"({payload_str})")
        self._frames += 1
    ...
    def _set_running(self, running: bool) -> None:
        ...
        if not running:
            ...
            self.sent_sub.setText("(waiting to start)")
